...
static void recordFieldGetter(Compiler* compiler, Variable classVariable, const char* fieldName, int fieldNameLength)
{
    Signature signature = signatureFromToken(compiler, SIG_GETTER);
    ...
    methodCompiler.isInitializer = false;
    ...
    ClassInfo* enclosingClass = getEnclosingClass(&methodCompiler);
    int field = wrenSymbolTableEnsure(methodCompiler.parser->vm, &enclosingClass->fields,
        fieldName,
        fieldNameLength);
    if (field >= MAX_FIELDS)
    {
    ...
    emitByteArg(&methodCompiler, CODE_LOAD_FIELD_THIS, field);
    emitOp(&methodCompiler, CODE_RETURN);
...
static void recordFieldSetter(Compiler* compiler, Variable classVariable, const char* fieldName, const char* fieldNameLength)
{
    Signature signature = signatureFromToken(compiler, SIG_SETTER);
    compiler->enclosingClass->signature = &signature;
    signature.arity = 1;
    ...
    int valueParameter = addLocal(&methodCompiler, "value", strlen("value"));
    ...
    loadLocal(&methodCompiler, valueParameter);
    emitByteArg(&methodCompiler, CODE_STORE_FIELD_THIS, field);
    emitOp(&methodCompiler, CODE_RETURN);
    endCompiler(&methodCompiler, fullSignature, length);
...
// Compiles a record field getter and setter in record definition
static bool recordField(Compiler* compiler, Variable classVariable, bool generateSetter)
{
    nextToken(compiler->parser);

    int number = compiler->enclosingClass->fields.count;
    char fieldName[MAX_VARIABLE_NAME];
    sprintf(fieldName, "%.*s", compiler->parser->previous.length, compiler->parser->previous.start); //"_<k>%d", number);
    int fieldNameLength = strlen(fieldName);

    recordFieldGetter(compiler, classVariable, fieldName, fieldNameLength);
    if (generateSetter)
        recordFieldSetter(compiler, classVariable, fieldName, fieldNameLength);

    return true;
}

// Compiles a record toString getter
static void recordToString(Compiler* compiler, Variable classVariable)
{
    const char* name = "toString";

    Signature signature;
    signature.name = name;
    signature.length = strlen(name);
    signature.type = SIG_GETTER;
    signature.arity = 0;
    compiler->enclosingClass->signature = &signature;
    ...
    loadCoreVariable(&methodCompiler, "List");
    callMethod(&methodCompiler, 0, "new()", 5);

    char beginning[MAX_VARIABLE_NAME + 2];
    sprintf(beginning, "%s(", enclosingClass->name->value);
    Value beginningValue = wrenNewStringLength(methodCompiler.parser->vm, beginning, strlen(beginning));
    emitConstant(&methodCompiler, beginningValue);
    callMethod(&methodCompiler, 1, "addCore_(_)", 11);

    for (int i = 0; i < enclosingClass->fields.count; i++)
    {
        char buffer[MAX_VARIABLE_NAME + 4];
        char* parameterName = buffer;
        if (i > 0)
        {
            sprintf(parameterName, ", ");
            parameterName += 2;
        }
        sprintf(parameterName, "%s=", enclosingClass->fields.data[i]->value);
        Value parameterNameValue = wrenNewStringLength(methodCompiler.parser->vm, buffer, strlen(buffer));
        emitConstant(&methodCompiler, parameterNameValue);
        callMethod(&methodCompiler, 1, "addCore_(_)", 11);

        emitByteArg(&methodCompiler, CODE_LOAD_FIELD_THIS, i);
        callMethod(&methodCompiler, 1, "addCore_(_)", 11);
    }

    const char* ending = ")";
    Value endingValue = wrenNewStringLength(methodCompiler.parser->vm, ending, strlen(ending));
    emitConstant(&methodCompiler, endingValue);
    callMethod(&methodCompiler, 1, "addCore_(_)", 11);

    // The list of interpolated parts.
    callMethod(&methodCompiler, 0, "join()", 6);
...
static void recordEquals(Compiler* compiler, Variable classVariable)
{
    // Compile ==
    Signature signature = { "==", 2, SIG_METHOD, 1 };
    compiler->enclosingClass->signature = &signature;
    ...
    int otherParameter = addLocal(&methodCompiler, "other", strlen("other"));

    // code here
    loadLocal(&methodCompiler, otherParameter);
    emitShortArg(&methodCompiler, CODE_LOAD_MODULE_VAR, classVariable.index);
    Signature isSignature = { "is", 2, SIG_METHOD, 1 };
    callSignature(&methodCompiler, CODE_CALL_0, &isSignature);

    for (int i = 0; i < enclosingClass->fields.count; i++)
    {
        int jump = emitJump(&methodCompiler, CODE_AND);

        ObjString* field = enclosingClass->fields.data[i];

        loadLocal(&methodCompiler, otherParameter);
        emitByteArg(&methodCompiler, CODE_LOAD_FIELD, i);

        emitByteArg(&methodCompiler, CODE_LOAD_FIELD_THIS, i);

        Signature equalsSignature = { "==", 2, SIG_METHOD, 1 };
        callSignature(&methodCompiler, CODE_CALL_0, &equalsSignature);

        patchJump(&methodCompiler, jump);
    }
    ...
    enclosingClass = getEnclosingClass(&methodCompiler);
    otherParameter = addLocal(&methodCompiler, "other", strlen("other"));

    loadThis(&methodCompiler);
    loadLocal(&methodCompiler, otherParameter);
    Signature eqSignature = { "==", 2, SIG_METHOD, 1 };
    callSignature(&methodCompiler, CODE_CALL_0, &eqSignature);
    callMethod(&methodCompiler, 0, "!", 1);
    emitOp(&methodCompiler, CODE_RETURN);
    endCompiler(&methodCompiler, fullSignature, length);
...
// Compiles a record constructor for no fields
static void recordConstructEmpty(Compiler* compiler, Variable classVariable)
{
    Signature signature = { "empty", 5, SIG_INITIALIZER, 0 };
    compiler->enclosingClass->signature = &signature;
    ...
    // Compile the method signature.
    methodCompiler.isInitializer = true;
    ...
    emitOp(&methodCompiler, CODE_LOAD_LOCAL_0);
    emitOp(&methodCompiler, CODE_RETURN);
    endCompiler(&methodCompiler, fullSignature, length);

    defineMethod(compiler, classVariable, false, methodSymbol);

    // Define constructor on metaclass
    signature.type = SIG_METHOD;
...
// Compiles a record constructor for all fields, in declared order
static void recordConstructNew(Compiler* compiler, Variable classVariable)
{
    // Don't compile a constructor that is impossible to call
    if (compiler->enclosingClass->fields.count > MAX_PARAMETERS) return;

    Signature signature = { "new", 3, SIG_INITIALIZER, compiler->enclosingClass->fields.count };
    compiler->enclosingClass->signature = &signature;
    ...
    {
        ObjString* field = enclosingClass->fields.data[i];
        int valueParameter = addLocal(&methodCompiler, field->value, field->length);
        loadLocal(&methodCompiler, valueParameter);
        emitByteArg(&methodCompiler, CODE_STORE_FIELD_THIS, i);
    }
...
// Compiles a `clone()` method on record that returns a shallow clone of this record
static void recordClone(Compiler* compiler, Variable classVariable)
{
    Signature signature = { "clone", 5, SIG_METHOD, 0 };
    compiler->enclosingClass->signature = &signature;
    ...
    emitShortArg(&methodCompiler, CODE_LOAD_MODULE_VAR, classVariable.index);
    Signature emptyConstructorSignature = { "empty", 5, SIG_METHOD, 0 };
    int emptyConstructorSymbol = signatureSymbol(&methodCompiler, &emptyConstructorSignature);
    emitShortArg(&methodCompiler, CODE_CALL_0, emptyConstructorSymbol);

    int instance = addLocal(&methodCompiler, "instance", strlen("instance"));

    for (int i = 0; i < enclosingClass->fields.count; i++)
    {
        emitByteArg(&methodCompiler, CODE_LOAD_FIELD_THIS, i);
        loadLocal(&methodCompiler, instance);
        emitByteArg(&methodCompiler, CODE_STORE_FIELD, i);
    }
...
// Record syntax: record Vector2(x, y) { ... }
// Compiles a method for each record field
// In this case: withX(value), withY(value), etc
// Each `withN` method returns a new record
static void recordWithMethods(Compiler* compiler, Variable classVariable)
{
    for (int i = 0; i < compiler->enclosingClass->fields.count; i++)
    {
        ObjString* field = compiler->enclosingClass->fields.data[i];

        char methodName[MAX_METHOD_NAME];
        sprintf(methodName, "with%c%s", (char)toupper(field->value[0]), field->value + 1);

        Signature signature = { methodName, strlen(methodName), SIG_METHOD, 1 };
        compiler->enclosingClass->signature = &signature;
        ...
        loadThis(&methodCompiler);
        Signature cloneSignature = { "clone", 5, SIG_METHOD, 0 };
        int cloneSymbol = signatureSymbol(&methodCompiler, &cloneSignature);
        emitShortArg(&methodCompiler, CODE_CALL_0, cloneSymbol);
...
// Compiles a record definition. Assumes the "record" token has already been
// consumed.
//
// Record example:
//  record Vector3(x, y, z) {
//    // additional getters, setters, methods...
//  }
static void recordDefinition(Compiler* compiler)
{
    bool generateSetters = !match(compiler, TOKEN_READONLY);
    ...
    consume(compiler, TOKEN_LEFT_PAREN, "Expect `(` after record");
    ...
    do
    {
        if (!recordField(compiler, classVariable, generateSetters)) break;
    } while (match(compiler, TOKEN_COMMA));

    consume(compiler, TOKEN_RIGHT_PAREN, "Expect `)` after record fields");

    // TODO: generate only if not already generated
    recordToString(compiler, classVariable);
    recordEquals(compiler, classVariable);
    recordConstructEmpty(compiler, classVariable);
    recordConstructNew(compiler, classVariable);
    recordClone(compiler, classVariable);
    recordWithMethods(compiler, classVariable);

    // Compile the method definitions.
    consume(compiler, TOKEN_LEFT_BRACE, "Expect '{' after record declaration.");
    matchLine(compiler);
    ...
    while (!match(compiler, TOKEN_RIGHT_BRACE))
    {
        ...
        consumeLine(compiler, "Expect newline after definition in record.");
    }
...
void definition(Compiler* compiler)
{
    ...
    else if (match(compiler, TOKEN_RECORD))
    {
